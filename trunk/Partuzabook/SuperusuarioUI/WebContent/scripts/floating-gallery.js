...
{	
	...
	tmpTop = e.clientY + Math.max(document.body.scrollTop,document.documentElement.scrollTop) - 10 ;
...
function startMoveTimer(){
	...
	if(moveTimer==10){
		...
		dragDropDiv.style.top = tmpTop + 'px' - 10;
		dragDropDiv.style.left = tmpLeft + 'px'  - 10;
...
function dragDropMove(e)
{
	...
	for(var prop in divXPositions){
		if(divXPositions[prop]/1 < leftPos/1 && (divXPositions[prop]/1 + divWidth[prop]*0.7)>leftPos/1 && divYPositions[prop]/1<topPos/1 && (divYPositions[prop]/1 + divWidth[prop])>topPos/1){
			...
			insertionMarker.style.top = divYPositions[prop] + offsetY + 'px' - 10;
...
//	document.getElementById('debug').innerHTML = 'This is the new order of the images(IDs) : <br>' + orderString;
	...
	document.getElementById('formThumb:imageIdList').value = orderString;
	document.getElementById('formThumb:btnUpdateOrder').click();
