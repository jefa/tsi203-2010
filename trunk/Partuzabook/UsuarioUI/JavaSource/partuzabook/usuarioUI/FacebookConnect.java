		...
		try {
			...
			if (!servicesUser.existsFacebookUser(userId)) 
			{
				...
				if(jo!=null){
				  ...
				  String email=jo.getString(ProfileField.PROXIED_EMAIL.toString());
				  userName = (firstName.substring(0,2) + lastName).toLowerCase();
				  // buscar una forma de que no se repitan los username
				  servicesUser.createNormalUser(userName,String.valueOf(UUID.randomUUID()),email , firstName + " " + lastName, userId); // uso uuid para que no se puedan loguear
				}
			}
			else
			{
